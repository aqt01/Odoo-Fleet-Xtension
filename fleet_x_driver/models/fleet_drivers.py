...
class fleet_driver(models.Model):
    ...
    identification_no = fields.Char(_('Identification #'))
    date_hired = fields.Date(_('Hire Date'), help=_('Date when the driver is hired'),
                             required=True, track_visibility='onchange')
    date_terminated = fields.Date(_('Terminated Date'),
                                  help=_('Date when the driver is terminated from job'),
                                  track_visibility='onchange', readonly=True)
    date_license_exp = fields.Date(_('License exp. Date'), help=_('Date when license expires'),
                                   required=False, track_visibility='onchange',
                                   inverse="_set_date_license_exp")
    dob = fields.Date(_('Date of Birth'))
    gender = fields.Selection([('male', _('Male')), ('female', _('Female'))], _('Gender'))
    vehicle_id = fields.Many2one('fleet.vehicle', string=_("Vehicle"), index=1,
                                 track_visibility='onchange', readonly=True,
                                 compute="_compute_vehicle", store=True)
    state = fields.Selection([('draft', _('Draft')),
                              ('unassigned', _('Unassigned')),
                              ('assigned', _('Assigned')),
                              ('license_exp', _('License Expired')),
                              ('terminated', 'Terminated')], 'State',
                             default='draft', track_visibility='onchange')
    previous_assignment_ids = fields.One2many('fleet.driver.assignment', 'driver_id',
                                              _("Previous Vehicles"), readonly=True)
    previous_assignment_count = fields.Integer(_('Assignment Count'),
    ...
    license_no = fields.Char(_('License Number'), required=True)
    issue_ids = fields.One2many('fleet.vehicle.issue', 'driver_id', _('Issues'),
                                readonly=True)
    issue_count = fields.Integer(_('Issue Count'), compute='_get_issue_count',
                                 readonly=True)
    location_id = fields.Many2one('fleet.vehicle.location', _('Operational Location'),
...
class fleet_vehicle(models.Model):
    ...
    vehicle_driver_id = fields.Many2one('fleet.driver', _('Assigned Driver (Primary)'),
                                        help=_('Primary driver of the vehicle'),
                                        track_visibility='onchange',
                                        inverse="_set_driver",
                                        domain=[('state', '=', 'unassigned')])
    alt_vehicle_driver_id = fields.Many2one('fleet.driver', _('Assigned Driver (Secondary)'),
                                            help=_('Secondary driver of the vehicle'),
                                            track_visibility='onchange',
                                            inverse="_set_alt_driver",
                                            domain=[('state', '=', 'unassigned')])
    driver_id = fields.Many2one('res.partner', _('Driver'), help=_('Driver of the vehicle'),
    ...
    previous_assignment_ids = fields.One2many('fleet.driver.assignment', 'vehicle_id',
                                              _("Previous Drivers"), readonly=True)
...
class fleet_driver_assignment(models.Model):
    ...
    vehicle_id = fields.Many2one('fleet.vehicle', _('Vehicle'), requried=True)
    driver_id = fields.Many2one('fleet.driver', _('Driver'), required=True)
    date_start = fields.Date(_('Start Date'), required=True, help=_('Vehicle assignment start-date'))
    date_end = fields.Date(_('End Date'), help=_('Vehicle assignment end-date'), default=False)
    notes = fields.Text(_('Notes'))
    odometer_start = fields.Float(_('Odometer at Start'), readonly=True)
    odometer_end = fields.Float(_('Odometer at Finish'), readonly=True)
    type = fields.Selection([('primary', _('Primary')), ('secondary', _('Secondary'))], required=True,
                            default='primary')
    ...
    def _check_driver_assign(self):
        ...
        if self.search_count(domain):
            raise Warning(_('Driver is already assigned to another vehicle'))
        ...
        if self.search_count(domain):
            raise Warning(_('Vehicle is already assigned to another driver of this same type'))


class fleet_vehicle_issue(models.Model):
    _inherit = 'fleet.vehicle.issue'

    driver_id = fields.Many2one('fleet.driver', _('Responsible'), required=True, readonly=True,
