    ...
    def post_cost(self):
        ...
        for cost in self:
            ...
            if float_compare(credit_sum, debit_sum, precision_digits=precision) == -1:
                ...
                if not acc_id:
                    raise Warning(_('The Expense Journal "%s" has not properly configured the Credit Account!') % (
                    journal_id.name))
                adjust_credit = (0, 0, {
                ...
                if not acc_id:
                    raise exceptions.UserError(
                        _('The Expense Journal "%s" has not properly configured the Debit Account! ').format(
                            cost.journal_id.name))
                adjust_debit = (0, 0, {
                    'name': _('Adjustment Entry'),
                    'date': timenow,
                    'partner_id': False,
...
class fleet_service_type(models.Model):
    ...
    analytic_account_id = fields.Many2one('account.analytic.account', _('Analytic Account'),
                                          domain=[('type', '!=', 'view')])
    account_debit = fields.Many2one('account.account', _('Debit Account'),
                                    domain=[('type', '=', 'other')])
    account_credit = fields.Many2one('account.account', _('Credit Account'),
                                     domain=[('type', '=', 'other')])
